    ...
    <el-container>
      <el-aside width="200px">
        ...
        >
          ...
          <el-menu-item index="/teapublished">
            <template slot="title">
              <i class="el-icon-document"></i>
              <span>已发布作业</span>
            </template>
          </el-menu-item>
          <el-menu-item index="/teamarking">
            <template slot="title">
              <i class="el-icon-edit-outline"></i>
              <span>未批阅作业</span>
            </template>
          </el-menu-item>
          <el-menu-item index="/teamarked">
            <template slot="title">
              <i class="el-icon-document-checked"></i>
              <span>已批阅作业</span>
            </template>
          </el-menu-item>
          <el-menu-item index="/teascore">
            <template slot="title">
  ...
  methods: {
    ...
    updatePassword() {
      this.$refs.ruleForm.validate(async valid => {
        ...
        const { data: info } = await this.$http.get('/teacher/getname', {
          params: { id: this.id }
        })
        ...
        const { data: res } = await this.$http.put('/teacher/password', {
